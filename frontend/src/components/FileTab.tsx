...
export function FileTab() {
  ...
  async function pickEnc(files: File[]) {
    const f = files[0]; setEncFile(f); setResultBlob(null); setThumb(null);
  ...
  async function pickDec(files: File[]) {
    const f = files[0]; setDecFile(f); setDecMeta(null); setDecBlob(null); setDecPw("");
    if (!f) return;
  ...
  async function doDec() {
    ...
    try {
      const blob = mode() === "local" ? (await workerApi.decryptFile(f, decPw())).blob : await backendApi.decryptFile(f, decPw());
  ...
  return (
    <div class="grid lg:grid-cols-2 gap-4">
      ...
      <div class="surface p-4">
        <div class="text-[13px] font-semibold text-[var(--color-fg)] mb-3">文件加密</div>
        <FileDrop zone="encrypt" label={encFile() ? encFile()!.name : "选择文件"} hint={encFile() ? `${formatBytes(encFile()!.size)} · ${guessMime(encFile()!)}` : "任意类型 · 流式分片处理"} onFiles={pickEnc} icon="🔒" />
        <Show when={encFile()}>
          <div class="mt-3 space-y-2.5">
            <div><div class="label">密码</div><input class="input" type="password" placeholder="加密密码" value={encPw()} onInput={(e) => setEncPw(e.target.value)} /></div>
            <div><div class="label">备注</div><input class="input" placeholder="写入文件头（可选）" value={encNote()} onInput={(e) => setEncNote(e.target.value)} /></div>
            <label class="flex items-center gap-2 text-[12px] text-[var(--color-fg)] cursor-pointer">
              <input type="checkbox" class="accent-[var(--color-accent)]" checked={embedThumb()} onChange={(e) => toggleThumb(e.target.checked)} />
              嵌入缩略图（图片/视频自动生成）
            </label>
            <Show when={thumb()}>
              <div class="flex items-center gap-2.5 rounded-md bg-black/30 border border-[var(--color-border)] p-2">
                <img src={`data:${thumb()!.mime};base64,${btoa(String.fromCharCode(...thumb()!.bytes))}`} class="w-12 h-12 rounded object-cover" alt="" />
                <div class="text-[11px] text-[var(--color-muted)]">
                  {thumb()!.width}×{thumb()!.height} · {formatBytes(thumb()!.bytes.length)}<br/><span class="text-[var(--color-accent)]">将嵌入文件头，可免密预览</span>
                </div>
              </div>
            </Show>
            <Show when={progress()}><ProgressBar done={progress()!.done} total={progress()!.total} phase={progress()!.phase} /></Show>
            <div class="flex gap-2">
              <button class="btn-primary flex-1" disabled={busy()} onClick={doEnc}>{busy() ? "加密中…" : "加密"}</button>
              <Show when={resultBlob()}><button class="btn-ghost" onClick={() => downloadBlob(resultBlob()!, encFile()!.name + ".enc")}>下载 .enc</button></Show>
            </div>
            ...
            <div class="text-[11px] text-[var(--color-muted)]">模式：{modeLabel()} · {mode() === "local" ? "明文不离开浏览器" : "上传至 Bun 后端"}</div>
          </div>
        </Show>
      </div>
      ...
      {/* DECRYPT */}
      <div class="surface p-4">
        <div class="text-[13px] font-semibold text-[var(--color-fg)] mb-3">文件解密 / 预览</div>
        <FileDrop zone="decrypt" label={decFile() ? decFile()!.name : "选择 .enc 文件"} hint={decFile() ? formatBytes(decFile()!.size) : "免密读取文件头信息"} onFiles={pickDec} icon="🔓" />
        <Show when={!decMeta() && decFile()}><Empty>正在读取文件头…</Empty></Show>
        <Show when={decMeta()}>
          <div class="mt-3 rounded-md bg-black/30 border border-[var(--color-border)] p-3">
            <div class="flex gap-3">
              <Show when={decMeta()!.thumbnailBase64} fallback={<div class="w-16 h-16 rounded bg-white/5 border border-[var(--color-border)] flex items-center justify-center text-[var(--color-muted)] text-xs">无图</div>}>
                <img src={`data:image/jpeg;base64,${decMeta()!.thumbnailBase64}`} class="w-16 h-16 rounded object-cover border border-[var(--color-border)]" alt="" />
              </Show>
            ...
            <Show when={decMeta()!.meta.note}><div class="mt-2 text-[11px] text-[var(--color-muted)]">备注：{decMeta()!.meta.note}</div></Show>
            <div class="mt-1.5 text-[10px] text-[var(--color-accent)]">↑ 免密读取，无需解密完整文件</div>
          </div>
        </Show>
        <Show when={decFile()}>
          <div class="mt-3 space-y-2.5">
            <div><div class="label">解密密码</div><input class="input" type="password" placeholder="密码" value={decPw()} onInput={(e) => setDecPw(e.target.value)} /></div>
            <div class="flex gap-2">
              <button class="btn-primary flex-1" disabled={decBusy()} onClick={doDec}>{decBusy() ? "解密中…" : "解密"}</button>
              <Show when={decBlob()}><button class="btn-ghost" onClick={() => downloadBlob(decBlob()!, decMeta()?.meta.originalName || "decrypted")}>下载原文件</button></Show>
            </div>
          </div>
        </Show>
      </div>
